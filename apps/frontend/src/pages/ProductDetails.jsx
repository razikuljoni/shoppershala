...
import { useProduct, useProductReviews, useCreateReview } from '@/hooks/useApi';
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((s) => (
        <button
          ...
          className="text-[rgba(255,255,255,0.2)] hover:text-[var(--color-warning)] transition-colors"
        >
          <Star
            size={20}
            className={s <= (hovered || value) ? 'text-[var(--color-warning)] fill-current' : ''}
          />
        </button>
      ))}
    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-3">
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="relative aspect-square rounded-[var(--radius)] overflow-hidden bg-[rgba(255,255,255,0.02)] border border-[var(--color-border)]"
          >
            {product.images?.[selectedImage] ? (
              ...
              <div className="w-full h-full flex items-center justify-center">
                <Package size={48} className="text-[var(--color-muted-foreground)] opacity-30" />
              </div>
            )}
          </motion.div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-[var(--color-muted-foreground)]">
              {avgRating.toFixed(1)} ({reviews?.totalReviews} reviews)
            </span>
          </div>
          ...
          {product.stock > 0 && (
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center border border-[var(--color-border)] rounded-lg overflow-hidden">
                <button
                  className="px-3 py-2 text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)] hover:bg-[rgba(255,255,255,0.04)] transition-colors"
                  onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                >
                  <Minus size={14} />
                </button>
                <span className="px-4 py-2 text-sm font-bold text-[var(--color-foreground)] border-x border-[var(--color-border)]">
                  {quantity}
                </span>
                <button
                  className="px-3 py-2 text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)] hover:bg-[rgba(255,255,255,0.04)] transition-colors"
              ...
              <Button
                ...
                size="icon"
                onClick={() => toggleWishlist(product._id, currentUser)}
              >
            ...
            <CardContent className="p-6">
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  .map(([k, v]) => (
                    <div key={k}>
                      <dt className="text-xs text-[var(--color-muted-foreground)] uppercase tracking-wider font-semibold">
                        {k}
                      </dt>
                      <dd className="mt-1 text-sm text-[var(--color-foreground)] font-medium">
                        {v}
                      </dd>
                    </div>
                  ))}
